...
uint32_t color = ((obj["defColor"].as<uint32_t>() > 0) ? obj["defColor"].as<uint32_t>() : 0x00FF00);
//unsigned long printRefresh = 0;
//unsigned long printTime = 1000;

SSD1306Wire displayOLED(0x3c, SDA, SCL);   // ADDRESS, SDA, SCL
NeoDigito display1 = NeoDigito(DIGITS, PIXPERSEG, NEO_PIN, NEO_GRB + NEO_KHZ800);
//NeoDigito display1 = NeoDigito(17, 2, 25);
...
void displayInit()
{
  ...
  if (obj["enable_neo"].as<bool>() == true)
  {
    Serial.print("{\"Init_neoDisplay\":"); Serial.print(obj["neo_pin"].as<int>()); Serial.println("}");
  ...
  if (obj["enable_oled"].as<bool>() == true)
  {
    displayOLED.init();
    ...
    displayOLED.drawString(0, 0, "Sensor Box");
    displayOLED.display();
    neo_digits_status = true;
    delay(100);
  }
...
void PrintOut()
{
  if (neo_digits_status)
  {
    ...
    if (obj["enable_neo"].as<bool>())
    {
      ...
      if (obj["type"].as<String>() == "ergo")
      {
        if (obj["enable_sensors"].as<bool>() == false)
        {
          t = msg["sensors"]["t"].as<int>();
          h = msg["sensors"]["h"].as<int>();
          uv = msg["sensors"]["uv"].as<float>();
          db = msg["sensors"]["db"].as<int>();
          lux = msg["sensors"]["lux"].as<unsigned long>();
          ppm = msg["sensors"]["ppm"].as<unsigned int>();
        }
